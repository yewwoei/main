...
public class SignUpCommandParser implements Parser<SignUpCommand> {
    ...
    public SignUpCommand parse(String args) throws ParseException {
        ArgumentMultimap argMultimap =
                ArgumentTokenizer.tokenize(args, PREFIX_USERNAME, PREFIX_PASSWORD, PREFIX_NAME,
                        PREFIX_PHONE, PREFIX_EMAIL);

        if (!arePrefixesPresent(argMultimap, PREFIX_USERNAME, PREFIX_PASSWORD, PREFIX_NAME,
                PREFIX_PHONE, PREFIX_EMAIL)
                || !argMultimap.getPreamble().isEmpty()) {
            throw new ParseException(String.format(MESSAGE_INVALID_COMMAND_FORMAT, SignUpCommand.MESSAGE_USAGE));
        }

        Username username = ParserUserUtil.parseUsername(argMultimap.getValue(PREFIX_USERNAME).get());
        Password password = ParserUserUtil.parsePassword(argMultimap.getValue(PREFIX_PASSWORD).get());
        Name name = ParserUserUtil.parseName(argMultimap.getValue(PREFIX_NAME).get());
        Phone phone = ParserUserUtil.parsePhone(argMultimap.getValue(PREFIX_PHONE).get());
        Email email = ParserUserUtil.parseEmail(argMultimap.getValue(PREFIX_EMAIL).get());
