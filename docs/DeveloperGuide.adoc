...
By: `Team W10-2`      Since: `Jun 2016`      Licence: `MIT`
...
[NOTE]
As a more OOP model, we can store a `Tag` list in `Address Book`, which `Restaurant` can reference. This would allow `Address Book` to only require one `Tag` object per unique `Tag`, instead of each `Restaurant` needing their own `Tag` object. An example of how such a model may look like is given below. +
 +
...
===== Aspect: The Possibility for abuse
The current implementation makes it possible any users to abuse the restaurant addressbook.
Users can add, edit and delete restaurants for all users. In the upcoming implementation, we are lookng to restrict it
to only users who log in. Additionally, an administrator account can be created to approve of such additions by users.
These will be in the future versions of the Makan Book.
...
there are three status - PENDING, ACCEPTED and CLEARED and the id is a 15 digits long string to identify the debt.
...
>>>>>>> bc00f9fe52fee994a6a1c17b326858f148ea5dcc
...
* (NOT IMPLEMENTED) `ListScheduleCommand` is specific to the logged in user. The command lists the user's schedule for a given NUS week. A parser is necessary.
